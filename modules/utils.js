...
export const formatCardNumber = (number) => {
  ...
  switch (type) {
    case 'amex':
      return sanitized.slice(0, 15).replace(/(\d{4})(\d{0,6})(\d{0,5})/g, '$1 $2 $3').trim();
    case 'visa':
    case 'mastercard':
    case 'discover':
      return sanitized.slice(0, 16).replace(/(\d{4})/g, '$1 ').trim();
    default:
...
export const formatExpiryDate = (date) => {
  ...
  return sanitized.length > 2 ? sanitized.replace(/(\d{2})(\d{1,2})/, '$1/$2') : sanitized;
};
